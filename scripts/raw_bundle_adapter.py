#!/usr/bin/env python3
"""Map mature extractor outputs into an Open Knowledge Studio Raw bundle.

This adapter does not parse PDFs, run OCR, transcribe media, summarize source
material, or write to wiki/.  Its only job is to preserve extractor output,
locators, assets, provenance, and visible quality limitations in one bundle.
"""
...
def package_markitdown(args: argparse.Namespace) -> Path:
    ...
    media_assets = extract_pptx_media(source, assets_dir)

    references = markdown_asset_references(document)
    unresolved = [
        reference
        for reference in references
        if not is_url(reference) and not (output / reference).is_file()
    ]
    packaged_document = neutralize_unresolved_images(document, set(unresolved))
    ...
    coverage_checks, coverage_status = coverage_report(
        {
            ...
            "embedded_media": (len(media_assets), len(media_assets)),
        }
    )
    if coverage_status == "partial":
    ...
    quality = {
        ...
        "asset_count": 1 + len(media_assets),
        "embedded_media_count": len(media_assets),
        "markdown_asset_references": len(references),
        "unresolved_asset_references": len(unresolved),
        "coverage_status": coverage_status,
        "coverage_checks": coverage_checks,
        "warnings": warnings,
        "human_fallback": "逐页核对正文；通过原PPT和assets/ppt-media核对图片、图表与排版",
    }
    write_json(output / "quality-report.json", quality)
    raw_markdown = f"""---
schema_version: {SCHEMA_VERSION}
...
- `assets/ppt-media/`：{len(media_assets)}个PPT内嵌媒体
...
def select_visual_summaries(
    ...
    for frame in frames:
        blocks = [
            ...
            for block in frame.get("ocr_blocks", [])
...
def package_watch_payload(
    ...
    *,
    ...
    if not digest:
        raise ValueError("无法为视频来源生成稳定指纹")
    info = payload.get("acquisition", {}).get("info", {})
    resolved_title = title or info.get("title") or (
        source_file.stem if source_file else Path(urlparse(source).path).stem or "video"
    )
    capture_id = f"{datetime.now():%Y%m%d}-video-{digest[:12]}"
    ...
    for frame in frames:
        ...
        for block_index, block in enumerate(frame.get("ocr_blocks", [])):
            ocr_count += 1
            evidence.append(
                {
    ...
    ) = render_watch_content(transcript_segments, frames, image_map)
    (output / "content.md").write_text(
    ...
    if not transcript_segments:
        ...
        warning_values.append("字幕或ASR逐字稿未经人工校对")
    ...
    quality = {
        ...
        "human_fallback": "抽样校对逐字稿；逐帧核对将用于Draft或Wiki的屏幕文字",
    }
    write_json(output / "quality-report.json", quality)
    raw_markdown = f"""---
schema_version: {SCHEMA_VERSION}
...
- [可读Raw正文](content.md)：{transcript_group_count}个语音段落，{content_visual_count}个视觉段落
- [未校对逐字稿](transcript.md)：{len(transcript_segments)}段
- [视觉证据](visual.md)：{len(image_map)}帧，{ocr_count}个OCR块
- [原子证据](evidence.jsonl)：{evidence_count}条
...
def main() -> int:
    ...
    if args.command == "mineru":
        print(package_mineru(args))
        return 0
    if args.command == "markitdown":
        print(package_markitdown(args))
        return 0
    if args.command == "watch":
        print(run_watch(args))
        return 0
