...
def promote_candidate_document(
    ...
    *,
    ...
    try:
        store = importlib.import_module("knowledge_studio.store")
    except ImportError as first_error:
        # A source checkout keeps the installable package at <repo>/cli, while
        # this optional reference implementation may move independently.  Find
        # that package by structure instead of coupling imports to *root*, which
        # is the worker state root and is routinely monkeypatched in tests.
        for parent in Path(__file__).resolve().parents:
            source_cli = parent / "cli"
            if not (source_cli / "knowledge_studio" / "store.py").is_file():
                continue
            source_cli_text = str(source_cli)
            if source_cli_text not in sys.path:
                sys.path.insert(0, source_cli_text)
            importlib.invalidate_caches()
            loaded_package = sys.modules.get("knowledge_studio")
            if loaded_package is not None and getattr(loaded_package, "__file__", None) is None:
                # A stale/broken namespace package can mask the regular source
                # package even after its parent is added to sys.path.
                sys.modules.pop("knowledge_studio", None)
            store = importlib.import_module("knowledge_studio.store")
            break
        else:
            raise RuntimeError(
                "knowledge_studio is not installed and no source checkout CLI was found"
            ) from first_error
