...
open-knowledge-studio 是一个面向 Claude Code 的知识工程仓库，提供 5 认知桶 + 2 基础设施层的记忆架构、6+1 因子召回引擎、衰减系统和 Dreaming 蒸馏管线。核心管线：raw → wiki → 检索/召回。
...
| `mail/` | 认知 | Agent 间短期通信 | 无 | 不参与 Recall |
...
- **召回引擎**: 6+1 因子相关性评分 + 记忆曲线
- **衰减系统**: 类型特定 λ + tier 分级
- **Claude Code**: 内置 skills（数量以当前 `assets/skills/` 为准）+ hooks + rules
...
oks drafts list/get/promote # 草稿工作流
oks distill                 # 蒸馏
