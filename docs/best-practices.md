...
1. 选择一份真正看过的资料
2. 入库并保留原始证据
3. 审核 Draft，再提升为 Wiki
4. 在真实问题中召回使用
```

**关键数据**：
- 召回结果：显示命中的 Wiki、相关性和来源
- 成本分析：区分单次实测、API 参考价格与实例化测算
- 技术方案：基于审核后的知识和可追溯证据
...
**核心收获**：从"看过这个视频"到"能用它做技术选型"，关键不是压缩成固定分钟数，而是保留证据、完成审核、验证召回。
...
### Q: 适合团队用吗？
**A**: 个人知识库用 `oks init`；需要多人共享、提交和审核同一套知识时，用 `oks team init` 创建共享实例，再绑定成员 profile 和团队目标。
