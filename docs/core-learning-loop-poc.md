...
Run 002 的用户审核意图明确为 `accept / 文章有价值 / 无修改`。用户没有被要求手工维护 Base 审核字段；Agent 根据明确指令回填唯一待审记录，再由 Worker 消费。人工决策和 Base 审核轨迹均真实存在，符合“Agent 总结和提问、用户判断、Agent 结构化落账并晋升”的职责边界。个人飞书发送和回复消费代码均已接入，发送与事件订阅分别完成真实探测；由于已发送的开发测试消息没有绑定 Candidate，仍需用一个新 Candidate 做一次回复到晋升的端到端验收。当前 Wiki trace 也仍含本机绝对 Raw 路径，来源可移植性尚未通过。
...
- [ ] 将本轮问题归为入口、获取、解析、Candidate、Wiki、召回或展示问题。
- [ ] 只选择影响最大的一项做最小修复。
- [ ] 用同一种子样本复跑修复前后对照。
