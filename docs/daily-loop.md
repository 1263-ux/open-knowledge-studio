...
**② 入料到 raw/**——先用 `oks ingest prepare` 生成协议 workspace；Agent 再按 Recipe 选择 Provider，把原始输出和 Evidence 提交为 Raw Bundle。这一步只做**模态转换与证据记录**，不做知识提炼。原则见 [Raw 多模态协议](raw-multimodal-standard.md)。
