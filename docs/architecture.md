...
| **衰减** | 不参与 Wiki 记忆衰减；实际保留周期由 Raw 存储策略决定 | 类型特定 λ — 知识随时间衰减 |
...
当 3+ 条 memory 共享同一标签且 score > 0.5 时，系统可以提出一篇合成参考文章 Candidate — **Crystal**。来源必须被引用；后续相关信息只能触发更新提案，仍需人工审核后才能替换正式 Wiki。
