...
成熟工具负责下载、字幕、ASR、OCR、关键帧和文档解析。本项目只负责来源适配、模态路由、格式映射、质量验证和 OKS 接入。
...
python scripts/raw_bundle_adapter.py markitdown "D:\sample\slides.pptx" `
  --output ".oks\intake\slides"

# 打包已有MinerU结果
python scripts/raw_bundle_adapter.py mineru "D:\mineru-output" `
  --source "D:\sample\paper.pdf" `
  --output ".oks\intake\paper"

# 使用RapidOCR环境提取独立图片
python scripts/raw_bundle_adapter.py image "D:\sample\screenshot.png" `
  --output ".oks\intake\screenshot"

# 所有包进入raw/前必须通过结构和证据校验
python scripts/raw_bundle_adapter.py validate ".oks\intake\lesson"
```

`watch` 命令应使用已安装Watch Skill的Python环境执行；`markitdown` 命令应使用已安装MarkItDown的环境执行。提取器缺失时命令明确失败，不会自动安装或偷偷切换到自研实现。
