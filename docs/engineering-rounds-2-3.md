...
> Historical engineering record: this document preserves the Round 2/3 implementation and test plan. Paths described as historical below are not current public entry points; current behavior is defined by the packaged Provider/Capability assets and the Agent-native ingest protocol.
...
- [x] **C -- Public-web Raw extractor moved to production (historical snapshot)**
  ...
  - Current v0.4 uses the packaged HTTP, Trafilatura, Firecrawl, browser, or Agent-native Provider contracts rather than exposing the legacy extractor as a public entry point.
...
- [x] **E -- Watch helper shadowing removed (historical snapshot)**
  ...
  - Current v0.4 exposes media behavior through packaged Provider/Capability contracts; the legacy module paths below remain only as historical implementation detail.
...
| File | Change |
|------|--------|
| `scripts/feishu_base_worker.py` | Historical retry, lease, and timestamp hardening; current worker behavior is split into `scripts/feishu_worker/`. |
| `scripts/extractors/*` | Historical intermediate extractor implementation; current public contracts are packaged Providers and Capabilities. |
