...
```bash
pipx install open-knowledge-studio && pipx ensurepath   # 1. install the canonical CLI source
...
- To update the canonical source cleanly: `pipx upgrade open-knowledge-studio`.
...
```bash
pipx install open-knowledge-studio && pipx ensurepath   # 1. 安装主线 CLI
...
- 更新主线源码时：`pipx upgrade open-knowledge-studio`。
