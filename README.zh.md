...
[文档站](https://1263-ux.github.io/open-knowledge-studio/) · [实验数据](#实测效果) · [复现](#复现) · [更新日志](./CHANGELOG.md)
...
Open Knowledge Studio（OKS）是一个开源的**文件化外部记忆**，面向编码 Agent。它不靠黑盒向量库，而是把知识存成一个可人工审计、git 版本化的文件系统——`profiles/`、`raw/`、`wiki/`、`drafts/`、`mail/`——Agent 用 `oks recall`、`oks wiki list`、`oks trace` 来浏览。内容经过一条可审计的流水线（来源 → 证据片段 → Manifest → Raw → Candidate → 人工审核 → Wiki），像真实记忆一样衰减，按需召回。完整介绍：[从这里开始](https://1263-ux.github.io/open-knowledge-studio/)。
...
- **一套文件系统装下所有记忆。** profiles、raw、wiki、drafts、mail 各占一个目录，有不同的信任边界。Agent 确定性地定位和操作上下文，就像开发者操作文件一样。→ [文件系统范式](https://1263-ux.github.io/open-knowledge-studio/concepts/file-system-paradigm.html) · [记忆模型](https://1263-ux.github.io/open-knowledge-studio/concepts/memory-model.html)
- **人工审核是唯一入口。** 原始材料 ≠ 结论，Candidate ≠ 长期知识。任何东西都不会自动晋升到 Wiki——每条持久记忆都经过人审。→ [Dreaming 周期](https://1263-ux.github.io/open-knowledge-studio/concepts/memory-model.html)
- **三层召回压制幻觉。** Node-BM25 召回*什么*匹配，Soul Boost 重排*什么到达* Agent（反模式 ×1.5、review bonus、generic 降权），Memory Curve 评分*多新鲜*——所以置信度永远不会盖过事实。→ [召回引擎](https://1263-ux.github.io/open-knowledge-studio/algorithms/recall-engine.html)
- **知识像真实记忆一样衰减。** 不用的页面沿着 hot → warm → cold → evictable 降温；用过的页面会浮现。`importance × e^(-λ×days) + ln(1+access) + pin_bonus`。→ [衰减系统](https://1263-ux.github.io/open-knowledge-studio/algorithms/decay-system.html)
- **每次召回都可观测。** 每个查询都保留各因子分数和匹配路径（`oks recall "<q>" --explain`）；每次注入都记到 `records/inject.jsonl`。结果看着不对时，你能看到是哪个因子产生的。→ [评估](https://1263-ux.github.io/open-knowledge-studio/algorithms/recall-evaluation.html)

各部分怎么拼一起：[架构](https://1263-ux.github.io/open-knowledge-studio/concepts/architecture.html)。设计背后的思考：[CONSTITUTION.md](./CONSTITUTION.md)。
...
> 💡 **第一次用 OKS？** 先读[从这里开始](https://1263-ux.github.io/open-knowledge-studio/)——它讲清记忆生命周期和 OKS 在你 Agent 栈里的位置。
...
- [Claude Code](https://1263-ux.github.io/open-knowledge-studio/usage/recall.html) —— `.claude/hooks/` + `settings.json`
- [Codex](https://1263-ux.github.io/open-knowledge-studio/usage/recall.html) —— `.codex/hooks.json`
- [qoder](https://1263-ux.github.io/open-knowledge-studio/usage/recall.html) —— `.qoder/settings.json`（共享 `.claude/hooks/`）
- [pi](https://1263-ux.github.io/open-knowledge-studio/usage/recall.html) —— `.pi/extensions/*.ts`（TS extension，共享 `.claude/hooks/`）
- [其他 shell](https://1263-ux.github.io/open-knowledge-studio/usage/recall.html) —— 任何能跑 shell hook 的宿主
...
- **文档**：[open-agent-power.github.io/open-knowledge-studio](https://1263-ux.github.io/open-knowledge-studio/)
