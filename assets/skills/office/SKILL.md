...
3. Recall relevant OKS knowledge to add team context, prior decisions, and
   ...
   leads, not evidence.
4. Build and validate one `oks-office-evidence/v1` package before prose or
   rendering. Read [the evidence contract](references/evidence-contract.md).
   The package can mix OKS and web sources, but every material claim maps to
   source ids and retains its review status.
...
6. Run package preflight, structural checks, and render-based visual QA.
   Deliver the editable artifact, readable source ledger, evidence package,
   and QA result. Never call a file "visually accepted" without inspecting the
   latest render.
...
available. The local `build_office.py` path is an explicitly labeled fallback,
not the production default. See [document workflow](workflows/document.md).

For PPTX, XLSX, and live desktop editing, use the same rule: prefer the
specialized host or optional adapter only after its local probe passes; retain
a portable fallback and honestly report environment limits.
